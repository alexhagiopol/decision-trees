function confusionMatrix = crossValidationWithDeletion(data, attributes)
...
for i = 1:10 %10 fold cross validation
    ...
    for j = 1:r
        if trainingDataTemp(j,5) ~= -1            
            if rand(1) < 0.2 % 20% chance that this example will have a missing attribute value
                attIndex = round(1+(length(attributes)-1)*rand(1)); %pick random attribute to delete
                trainingDataTemp(j,attributes(attIndex)) = NaN; %delete data by setting equal to NaN
